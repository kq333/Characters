      ...
      <div class="table__content__Add">
        <button
          ...
          @click="
            ...
            findBtnId(item.id);
          "
          :class="[addClassColor(item.id) ? ' active ' : ' ']"
        >
...
export default {
  ...
  setup() {
    ...
    const changeColor = computed(() => store.state.BtnId);
  //  const addedToFav = computed(store.state.AddToFav);
    const getIconName = computed(() => store.state.iconName);
    const propsValue = computed(() => store.state.GraphQlPageNum);
    const Active = ref(false);

    watch(propsValue, (num) => {
      if (num) {
        updatedData();
      }
    });
    ...
    function addToFavorites(item) {
      const clickedValues = item;
      this.store.commit("addandRemoveValues", clickedValues);
    }

    function findBtnId(id) {
      if (changeColor.value.indexOf(id) == -1) {
        this.store.commit("changeColorBtn", id);
      } else {
        this.store.commit("removeColorFromBtn", id);
      }
    }

    function addClassColor(id) {
      return changeColor.value.includes(id);
    }
    ...
    function inputValue() {
      if (searchedBy.value === "Name") {
        return store.state.Arrs.filter((item) =>
